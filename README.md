...
```json
{
  "days": {
    "2026-09-05": 3,
    "2026-09-06": { "add": 2 },
    "2026-09-07": { "count": 1, "note": "只发了一张" },
    "2026-09-08": { "note": "写了一天代码" }
  }
}
```
...
| `3` | 覆盖成 3 张 |
| `{ "add": 2 }` | 在自动统计之上再加 2 |
| `{ "count": 1, "note": "…" }` | 覆盖成 1 张，并留一句备注 |
| `{ "note": "…" }` | 只留备注，张数按自动统计算 |

> ⚠️ **JSON 里不能写 `//` 注释**，写了整份文件就解析不过，
> 脚本会打印「daily.json 解析失败，已忽略」，你的备注会全部消失。
> 想留注释就写成下划线开头的键，比如 `"_说明": "…"`，脚本会跳过它。

- 值写**数字** = 覆盖那天的数量。
- 值写 `{ "add": n }` = 自动统计 + n。
- 值写 `{ "count": n }` = 覆盖。
- `note` 会显示在热力图的悬浮提示和「备注」块里。
- **只有备注、没有张数**的日子算「有记录」（格子是最浅的一档「低」），
  会算进「有记录的天数」和「连续天数」。
...
| 情况 | 表现 |
|---|---|
| **「备注」块和 `images/备注/` 是两回事** | 备注文字存在根目录的 `daily.json` 里；`images/备注/` 只是预留的占位大类（以后放页面用）。两块共用一个名字，但数据不相通 |
| **只写了备注、没写张数** | 张数按 0 算，但格子显示成最浅的「低」而不是「无」，也算进「有记录的天数」和「连续天数」——不然那天看起来像啥也没干 |
| **既写了张数又写了备注** | 两个块里都会出现：「最近记录」列它有几张，「备注」列它写了什么 |
| **只显示最近 4 条** | 备注块是「最近几条」的预览，不是完整档案；更早的备注只存在 `daily.json` 里 |
| **不受 3 个月 / 半年 / 一年影响** | 备注块永远列最近 4 条，跟上面选的窗口无关；某条备注对应的格子可能已经不在画面里 |
| **在看图的页面上看不到** | 整个右侧栏只出现在大类 / 图集列表页；进到图集里看图时它会让位给图片墙，点面包屑退回上一层就能看到 |
| **JSON 不能写 `//` 注释** | 写了整份 `daily.json` 直接解析失败、备注全部消失（脚本会打印一行黄字提醒）。想写注释就用 `_说明` 这类下划线开头的键 |
| **日期格式** | 键要写成 `2026-09-05`（`2026/9/5` 也认）。写错的键会被单独跳过并提醒，不会连累整个文件 |
| **改了要重跑脚本** | 备注不经过图片，但一样要靠 `tools\update-gallery.cmd` 写进 `heatmap.json`；开着自动同步的话存盘就会自动同步（它盯着 `daily.json`） |
...
- **只提交图片相关的东西**：`images/`、`thumbs/`、`views/`、`images.json`、
  `heatmap.json`、`daily.json`。代码、文档、别的杂项它一概不碰 ——
  你（或者帮你改代码的人）改到一半的东西不会被顺手推上线，
  那些改动会留在工作区等你自己 `git add .` 提交。
  日志里会写一行「另有 N 个改动不属于自动同步范围」。
  > 所以：**改完 `style.css` / `script.js` 要自己提交**。生成脚本会顺带改
  > `index.html` 里的资源版本号（防缓存），那个也不在自动同步范围内。
  > 想让它回到「什么都提交」的老行为：`node tools/auto-push.mjs --all-paths`

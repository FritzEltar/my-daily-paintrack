...
| 第 3 屏 | **图集内部** | 一整面正方形缩略图墙，无缝拼接；**默认按日期由新到旧**；悬停显示文件名，点开看原图 |
...
图集里默认**按日期由新到旧**，最新的排最前面。搜索框右边就是排序开关：

```
[ 在本图集内搜索标题、标签… ]        [ 由新至旧 ] [ 由旧至新 ]
```

- **由新至旧**（默认）：09-18 的图排在 09-17 的图前面。
- **由旧至新**：整体反过来。
- 排序用的是每张图的 `date`，和热力图是同一个值（来源见上面「每日热力图」那节）。
- **同一天**的图按文件名排；**没有日期的图**永远排最后。
- 只影响图集内部的墙，以及灯箱里 `←` `→` 翻页的顺序；
  图集封面、大类列表、图集列表都不受影响。
- 切换排序会跳回第 1 页；刷新页面后回到默认的「由新至旧」。

所以想让某张图排到前面，不用改文件名 —— 在 `images/meta.json` 里把它的 `date`
写成实际作画日期就行。
